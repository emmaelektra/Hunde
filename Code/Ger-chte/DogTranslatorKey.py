...
def takecommand():
    ...
    with sr.Microphone() as source:
        print("listening.....")
        listening = "Listening"
        speak_listen = gTTS(text=listening, lang='en', slow=False)
        speak_listen.save("SpeakListen.mp3")
        playsound('SpeakListen.mp3')
        os.remove('SpeakListen.mp3')
        r.pause_threshold = 1
    ...
    try:
        ...
        speak_listen = gTTS(text=recognizing, lang='en', slow=False)
        speak_listen.save("SpeakRecognize.mp3")
        playsound('SpeakRecognize.mp3')
        os.remove('SpeakRecognize.mp3')
...
def Translate():
    ...
    speak_indices_left = list(range(0,end,8))
    speak_indices_right = list(range(4,end+2,8))
    print(speak_indices_left,speak_indices_right)
    ...
    for n in range(0,end,2):
        ...
        stereo_audio = AudioSegment.from_file("audioCap.mp3")

        play_left = stereo_audio.pan(-1)
        play_right = stereo_audio.pan(1)

        # Using OS module to run the translated voice.
        if n in speak_indices_left:
            play(play_left)

        elif n in speak_indices_right:
            play(play_right)
